...
func GeneratePDF(schedule model.Schedule, admins []model.Admin, times []model.DayTime, dayLength int) error {
	...
	pdfDoc.AddPage(wkhtmltopdf.NewPageReader(strings.NewReader(generateHtmlCode(schedule, admins, times, dayLength))))
	pdfDoc.Dpi.Set(600)
	...
	prepareOutDir()
	err = pdfDoc.WriteFile(getOutputFile("pdf"))
	if err != nil {
		return err
	}
	return nil
